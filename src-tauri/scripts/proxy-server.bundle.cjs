...
`;
          ...
          let { host, port: port2, secureEndpoint } = opts;
          if (!isDefaultPort(port2, secureEndpoint)) {
            host += `:${port2}`;
          }
          headers.Host = host;
...
`);
    ...
    function isDefaultPort(port2, secure) {
      return Boolean(!secure && port2 === 80 || secure && port2 === 443);
    }
...
// node_modules/.pnpm/@rinova+proxy-sdk@2.0.0/node_modules/@rinova/proxy-sdk/dist/utils.js
var padBase64 = (s) => s.padEnd(s.length + (4 - s.length % 4) % 4, "=");
var safeDecodeURI = (s) => {
  if (!s)
    return "";
  try {
...
// node_modules/.pnpm/@rinova+proxy-sdk@2.0.0/node_modules/@rinova/proxy-sdk/dist/locales/en.json
var en_default = {
  fetching: "Fetching subscription:",
  parsed: "Parsed: {count} nodes",
  node_list: "{index}. [{type}] {server}:{port} \u2190 {name}",
  config_written: "Config written: {path}",
  config_summary: "{nodes} nodes, {groups} groups",
  merged: "Merged into: {path}",
  group_fallback: 'Group "{group}" references missing node "{name}", falling back to "{fallback}"',
  ...
  first_fetch_fail: "Initial fetch failed: {msg}",
  port_in_use: "Port {port} is already in use",
  skip_node: "Skipping unparseable node: {msg}",
  mode_requires_url: "Serve mode requires -u",
  no_nodes: "No valid proxy nodes found",
  shutting_down: "Shutting down...",
  http_start: "Starting HTTP service, refreshes every {interval} minutes",
  server_title: "Proxy Server",
  server_banner_clash: "Clash:",
  server_banner_health: "Health:",
  err_empty_subscription: "Subscription returned empty result",
  err_ss_address: "Invalid SS address: {host}",
  err_ss_sip002_credentials: "Invalid SS SIP002 credentials: {raw}",
  err_ss_legacy_no_at: "SS legacy format missing @: {raw}",
  err_ss_legacy_address: "SS legacy invalid address: {host}",
  err_ss_legacy_no_method: "SS legacy missing method: {raw}",
  err_vmess_parse: "VMess JSON parse failed",
  err_vmess_no_server: "VMess missing server or port",
  err_unsupported_protocol: "Unsupported protocol: {prefix}",
  err_no_nodes_subscription: "No valid proxy nodes found in subscription",
  err_no_nodes_input: "No valid proxy nodes found in the input"
};

// node_modules/.pnpm/@rinova+proxy-sdk@2.0.0/node_modules/@rinova/proxy-sdk/dist/locales/zh.json
var zh_default = {
  fetching: "\u62C9\u53D6\u8BA2\u9605\uFF1A",
  parsed: "\u89E3\u6790\u6210\u529F\uFF1A{count} \u4E2A\u8282\u70B9",
  node_list: "{index}. [{type}] {server}:{port} \u2190 {name}",
  config_written: "\u914D\u7F6E\u5DF2\u751F\u6210\uFF1A{path}",
  config_summary: "{nodes} \u4E2A\u8282\u70B9\uFF0C{groups} \u4E2A\u7B56\u7565\u7EC4",
  merged: "\u5DF2\u5408\u5E76\u5230\uFF1A{path}",
  group_fallback: '\u7B56\u7565\u7EC4 "{group}" \u5F15\u7528\u7684\u8282\u70B9 "{name}" \u5DF2\u4E0D\u5B58\u5728\uFF0C\u56DE\u9000\u5230 "{fallback}"',
  refreshing: "\u5237\u65B0\u8BA2\u9605...",
  refresh_ok: "\u5237\u65B0\u6210\u529F \u2014 {count} \u4E2A\u8282\u70B9",
  refresh_skip: "\u5237\u65B0\u5DF2\u5728\u8FDB\u884C\u4E2D\uFF0C\u8DF3\u8FC7",
  refresh_fail: "\u5237\u65B0\u5931\u8D25\uFF0C\u4FDD\u7559\u4E0A\u6B21\u7F13\u5B58\uFF1A{msg}",
  first_fetch_fail: "\u9996\u6B21\u62C9\u53D6\u5931\u8D25\uFF1A{msg}",
  port_in_use: "\u7AEF\u53E3 {port} \u5DF2\u88AB\u5360\u7528",
  skip_node: "\u8DF3\u8FC7\u65E0\u6CD5\u89E3\u6790\u7684\u8282\u70B9\uFF1A{msg}",
  mode_requires_url: "\u670D\u52A1\u6A21\u5F0F\u9700\u8981 -u \u53C2\u6570",
  no_nodes: "\u672A\u627E\u5230\u6709\u6548\u8282\u70B9",
  shutting_down: "\u5173\u95ED\u670D\u52A1...",
  http_start: "\u542F\u52A8 HTTP \u670D\u52A1\uFF0C\u6BCF {interval} \u5206\u949F\u81EA\u52A8\u5237\u65B0",
  server_title: "\u4EE3\u7406\u670D\u52A1",
  server_banner_clash: "\u8BA2\u9605\u5730\u5740\uFF1A",
  server_banner_health: "\u5065\u5EB7\u68C0\u67E5\uFF1A",
  err_empty_subscription: "\u8BA2\u9605\u94FE\u63A5\u8FD4\u56DE\u4E3A\u7A7A\u6216\u683C\u5F0F\u65E0\u6CD5\u8BC6\u522B",
  err_ss_address: "SS \u5730\u5740\u683C\u5F0F\u9519\u8BEF\uFF1A{host}",
  err_ss_sip002_credentials: "SS SIP002 \u51ED\u636E\u683C\u5F0F\u9519\u8BEF\uFF1A{raw}",
  err_ss_legacy_no_at: "SS legacy \u683C\u5F0F\u65E0\u6CD5\u627E\u5230 @\uFF1A{raw}",
  err_ss_legacy_address: "SS legacy \u5730\u5740\u683C\u5F0F\u9519\u8BEF\uFF1A{host}",
  err_ss_legacy_no_method: "SS legacy \u7F3A\u5C11 method\uFF1A{raw}",
  err_vmess_parse: "VMess JSON \u89E3\u6790\u5931\u8D25",
  err_vmess_no_server: "VMess \u7F3A\u5C11 server \u6216 port",
  err_unsupported_protocol: "\u4E0D\u652F\u6301\u7684\u534F\u8BAE\u7C7B\u578B\uFF1A{prefix}",
  err_no_nodes_subscription: "\u8BA2\u9605\u4E2D\u672A\u627E\u5230\u6709\u6548\u4EE3\u7406\u8282\u70B9",
  err_no_nodes_input: "\u8F93\u5165\u4E2D\u672A\u627E\u5230\u6709\u6548\u4EE3\u7406\u8282\u70B9"
};

// node_modules/.pnpm/@rinova+proxy-sdk@2.0.0/node_modules/@rinova/proxy-sdk/dist/i18n.js
...
// node_modules/.pnpm/@rinova+proxy-sdk@2.0.0/node_modules/@rinova/proxy-sdk/dist/fetch.js
var fetchSubscription = async (url3) => {
  const resp = await axios_default.get(url3, {
    timeout: 15e3,
    responseType: "text",
    headers: {
...
// node_modules/.pnpm/@rinova+proxy-sdk@2.0.0/node_modules/@rinova/proxy-sdk/dist/parser.js
var parseSS = (uri) => {
  ...
  if (atIdx >= 0) {
    ...
    const hostPort2 = body.slice(atIdx + 1);
    const [server2, portStr3] = hostPort2.split(":");
    const port3 = parseInt(portStr3, 10);
    if (!server2 || isNaN(port3))
      throw new Error(t("err_ss_address", { host: hostPort2 }));
    const decoded2 = Buffer.from(padBase64(b64Part), "base64").toString("utf-8");
    const atInDecoded2 = decoded2.indexOf("@");
    if (atInDecoded2 >= 0) {
      const [method2, ...pwdParts2] = decoded2.slice(0, atInDecoded2).split(":");
      return { name: rawName || server2, type: "ss", server: server2, port: port3, cipher: method2 || "none", password: pwdParts2.join(":") };
    }
    const colonIdx = decoded2.indexOf(":");
    if (colonIdx === -1)
      throw new Error(t("err_ss_sip002_credentials", { raw: decoded2 }));
    return { name: rawName || server2, type: "ss", server: server2, port: port3, cipher: decoded2.slice(0, colonIdx), password: decoded2.slice(colonIdx + 1) };
  }
  const decoded = Buffer.from(padBase64(body), "base64").toString("utf-8");
  const atInDecoded = decoded.indexOf("@");
  if (atInDecoded === -1)
    throw new Error(t("err_ss_legacy_no_at", { raw: decoded }));
  const cred = decoded.slice(0, atInDecoded);
  const hostPort = decoded.slice(atInDecoded + 1);
  const [server, portStr2] = hostPort.split(":");
  const port2 = parseInt(portStr2, 10);
  if (!server || isNaN(port2))
    throw new Error(t("err_ss_legacy_address", { host: hostPort }));
  const [method, ...pwdParts] = cred.split(":");
  if (!method)
    throw new Error(t("err_ss_legacy_no_method", { raw: cred }));
  return { name: rawName || server, type: "ss", server, port: port2, cipher: method, password: pwdParts.join(":") };
};
var parseVMess = (uri) => {
  ...
  try {
    ...
    throw new Error(t("err_vmess_parse"));
  }
  const node = {
  ...
  if (!node.server || !node.port)
    throw new Error(t("err_vmess_no_server"));
  const net = obj.net || "tcp";
  if (net === "ws") {
    node.network = "ws";
    const wsOpts = { path: obj.path || "/" };
    if (obj.host)
      wsOpts.headers = { Host: obj.host };
    node["ws-opts"] = wsOpts;
  } else if (net === "grpc") {
    node.network = "grpc";
    const grpcOpts = {};
    ...
    node.network = "h2";
    const h2Opts = { path: obj.path || "/" };
    if (obj.host)
      h2Opts.host = obj.host;
    node["h2-opts"] = h2Opts;
  } else if (net === "quic") {
    node.network = "quic";
    const quicOpts = { security: obj.security || "none", key: obj.key || "" };
  ...
  if (obj.tls === "tls") {
    ...
    if (obj.sni)
      node["sni"] = obj.sni;
    if (obj.alpn)
      node.alpn = obj.alpn?.split(",").map((s) => s.trim());
    if (obj.fp)
      node["client-fingerprint"] = obj.fp;
...
var parseHysteria2 = (uri) => {
  ...
  if (up)
    node.up = parseInt(up, 10);
  if (down)
    node.down = parseInt(down, 10);
  return node;
};
var parseURI = (uri) => {
...
// node_modules/.pnpm/js-yaml@5.2.1/node_modules/js-yaml/dist/js-yaml.mjs
...
function isPlainObject2(data) {
  ...
  const prototype2 = Object.getPrototypeOf(data);
...
// node_modules/.pnpm/@rinova+proxy-sdk@2.0.0/node_modules/@rinova/proxy-sdk/dist/builder.js
var BUILTIN_RULES = [
  "DOMAIN-SUFFIX,ad.com,\u{1F6D1} \u5E7F\u544A\u62E6\u622A",
  "DOMAIN-SUFFIX,doubleclick.net,\u{1F6D1} \u5E7F\u544A\u62E6\u622A",
  "DOMAIN-SUFFIX,google.com,\u{1F30D} \u56FD\u5916\u7F51\u7AD9",
  "DOMAIN-SUFFIX,youtube.com,\u{1F30D} \u56FD\u5916\u7F51\u7AD9",
  "DOMAIN-SUFFIX,github.com,\u{1F30D} \u56FD\u5916\u7F51\u7AD9",
  "DOMAIN-SUFFIX,twitter.com,\u{1F30D} \u56FD\u5916\u7F51\u7AD9",
  "DOMAIN-SUFFIX,x.com,\u{1F30D} \u56FD\u5916\u7F51\u7AD9",
  "DOMAIN-SUFFIX,telegram.org,\u{1F30D} \u56FD\u5916\u7F51\u7AD9",
  "DOMAIN-SUFFIX,steampowered.com,\u{1F30D} \u56FD\u5916\u7F51\u7AD9",
  "DOMAIN-SUFFIX,openai.com,\u{1F30D} \u56FD\u5916\u7F51\u7AD9",
  "DOMAIN-SUFFIX,anthropic.com,\u{1F30D} \u56FD\u5916\u7F51\u7AD9",
  "DOMAIN-SUFFIX,claude.ai,\u{1F30D} \u56FD\u5916\u7F51\u7AD9",
  "DOMAIN-SUFFIX,cursor.com,\u{1F30D} \u56FD\u5916\u7F51\u7AD9",
  "DOMAIN-SUFFIX,cloudflare.com,\u{1F30D} \u56FD\u5916\u7F51\u7AD9",
  "DOMAIN-SUFFIX,netflix.com,\u{1F30D} \u56FD\u5916\u7F51\u7AD9",
  "DOMAIN-SUFFIX,spotify.com,\u{1F30D} \u56FD\u5916\u7F51\u7AD9",
  "DOMAIN-SUFFIX,disneyplus.com,\u{1F30D} \u56FD\u5916\u7F51\u7AD9",
  "DOMAIN-SUFFIX,cn,\u{1F1E8}\u{1F1F3} \u56FD\u5185\u7F51\u7AD9",
  "DOMAIN-SUFFIX,baidu.com,\u{1F1E8}\u{1F1F3} \u56FD\u5185\u7F51\u7AD9",
  "DOMAIN-SUFFIX,qq.com,\u{1F1E8}\u{1F1F3} \u56FD\u5185\u7F51\u7AD9",
  "DOMAIN-SUFFIX,weixin.com,\u{1F1E8}\u{1F1F3} \u56FD\u5185\u7F51\u7AD9",
  "DOMAIN-SUFFIX,alipay.com,\u{1F1E8}\u{1F1F3} \u56FD\u5185\u7F51\u7AD9",
  "GEOIP,CN,\u{1F3AF} \u76F4\u8FDE",
  "MATCH,\u{1F30D} \u56FD\u5916\u7F51\u7AD9"
];
var EXT_RULES = [
  "RULE-SET,https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/BanEasyAD.list,\u{1F6D1} \u5E7F\u544A\u62E6\u622A",
  "RULE-SET,https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/ProxyMedia.list,\u{1F30D} \u56FD\u5916\u7F51\u7AD9",
  "RULE-SET,https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/ChinaDomain.list,\u{1F1E8}\u{1F1F3} \u56FD\u5185\u7F51\u7AD9",
  "GEOIP,CN,\u{1F3AF} \u76F4\u8FDE",
  "MATCH,\u{1F30D} \u56FD\u5916\u7F51\u7AD9"
];
var buildConfig = (nodes, ruleMode = "builtin") => {
  const names = nodes.map((n) => n.name);
  const foreignProxies = ["\u{1F680} \u8282\u70B9\u9009\u62E9", "\u267B\uFE0F \u81EA\u52A8\u9009\u62E9", "\u{1F3AF} \u76F4\u8FDE", "DIRECT", ...names];
  return {
    port: 7890,
    "socks-port": 7891,
    "allow-lan": true,
    ...
    proxies: nodes,
    "proxy-groups": [
      { name: "\u{1F680} \u8282\u70B9\u9009\u62E9", type: "select", proxies: ["\u267B\uFE0F \u81EA\u52A8\u9009\u62E9", "\u{1F3AF} \u76F4\u8FDE", "DIRECT", ...names] },
      { name: "\u267B\uFE0F \u81EA\u52A8\u9009\u62E9", type: "url-test", proxies: names, url: "http://www.gstatic.com/generate_204", interval: 300 },
      { name: "\u{1F3AF} \u76F4\u8FDE", type: "select", proxies: ["DIRECT", ...names] },
      { name: "\u{1F6D1} \u5E7F\u544A\u62E6\u622A", type: "select", proxies: ["REJECT", "DIRECT", ...names] },
      { name: "\u{1F30D} \u56FD\u5916\u7F51\u7AD9", type: "select", proxies: foreignProxies },
      { name: "\u{1F1E8}\u{1F1F3} \u56FD\u5185\u7F51\u7AD9", type: "select", proxies: ["\u{1F3AF} \u76F4\u8FDE", ...names] }
    ],
    rules: ruleMode === "external" ? EXT_RULES : BUILTIN_RULES
  };
};
var toYaml = (config) => dump(config, {
  indent: 2,
  lineWidth: -1,
  noRefs: true
});

// node_modules/.pnpm/@rinova+proxy-sdk@2.0.0/node_modules/@rinova/proxy-sdk/dist/server.js
var import_node_http = __toESM(require("node:http"), 1);
var startServer = async (opts) => {
  let cache = null;
  ...
  const refresh = async () => {
    ...
    refreshing = true;
    try {
      console.log(`[${(/* @__PURE__ */ new Date()).toLocaleTimeString()}] \u{1F504} ${t("refreshing")}`);
  ...
  console.log(`\u{1F517} ${t("refreshing")}`);
  await refresh();
  console.log(`\u{1F4E1} ${t("http_start", { interval: opts.intervalMin })}
`);
  const refreshTimer = setInterval(() => {
    refresh().catch(() => {
    });
  }, opts.intervalMin * 6e4);
  const server = import_node_http.default.createServer((req, res) => {
    ...
    if (pathname === "/clash.yaml") {
      ...
      res.end(JSON.stringify({
        ...
        nextRefreshMin: cache?.updatedAt ? Math.max(0, opts.intervalMin - Math.floor((now - cache.updatedAt) / 6e4)) : opts.intervalMin,
      ...
      refresh().then(({ skipped }) => {
        res.writeHead(200, { "Content-Type": "application/json; charset=utf-8" });
        res.end(JSON.stringify({ ok: true, skipped, nodes: cache?.nodeCount ?? 0 }));
      }).catch(() => {
...
// src-tauri/scripts/proxy-server.mjs
var [portStr, url2] = process.argv.slice(2);
...
startServer({ url: url2, port, intervalMin: 60, ruleMode: "builtin" }).then((server) => {
